...
正式部署建议用仓库里的脚本安装 systemd 服务。

```bash
cd /data/ysmjjsy/YSMJJSY
sudo bash scripts/deploy.sh
```

第一次运行时，如果 `/etc/ysmjjsy.env` 不存在，脚本会创建模板并退出。填好 `NUXT_MINIMAX_API_KEY` 和 `NUXT_MINIMAX_GROUP_ID` 后，再运行一次。
...
Type=simple
User=root
WorkingDirectory=/data/ysmjjsy/YSMJJSY
EnvironmentFile=/etc/ysmjjsy.env
