...
describe('agent timeline api helpers', () => {
  it('builds a sorted agent timeline from memories, reflections, proposals, sleep runs, and works', () => {
    const timeline = buildAgentTimeline({
      ...
      memories: [{ id: 'm1', content: '用户喜欢短句。', importance: 0.9, createdAt: '2026-05-17T00:03:00.000Z' }],
    ...
    expect(timeline.items.map(item => item.id)).toEqual(['s1', 'r1', 'w1', 'm1', 'p1', 'profile', 'key'])
    expect(timeline.items.map(item => item.createdAt)).toEqual([
    ...
    expect(timeline.items.find(item => item.id === 'm1')).toMatchObject({
      targetId: 'm1',
      targetType: 'memory',
      importance: 'high',
    })
